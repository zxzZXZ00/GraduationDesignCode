...
class AllImg():
    ...
    #初始化函数
    def __init__(self,file_paths,light_sum,save_path,limit,size):
    ...
    def import_frame(self):
        if self.cur_imp_frame < self.total:
            self.imgs.append(OneImg(self.file_paths[self.cur_imp_frame],self.cur_imp_frame,self.tags,self.light_sum,self.limit,self.size))
    ...
    def arr_frame(self,x,y):
        ...
        if x > 10 and y>10 and x < self.imgs[0].image.shape[0]-10 and y < self.imgs[0].image.shape[1]-10:
            self.num += 1
            self.new_path = f"{self.save_path}/{self.num}_{self.cur_pro_frame}_{x}_{y}"
            os.makedirs(self.new_path)
            for i in range(min(self.cur_pro_frame+150,300,self.total-self.cur_del_frame)):
                self.new_imgs[i].save(f"{self.new_path}/{i}.tiff")
    #导出一张图片
    def one_frame(self, x, y, frame):
        self.top = max(0, x-self.size)
        self.bottom = min(x+self.size, self.imgs[frame].image.shape[0]-1)
        self.left = max(0, y-self.size)
        self.right = min(y+self.size, self.imgs[frame].image.shape[1]-1)
...
class OneImg():
    ...
    def __init__(self,file_path,frame,tags,light_sum,limit,size):
        self.light_sum = light_sum
        self.limit = limit
        self.size = size
    ...
    def get_light_addr(self,frame,tags):
        # 初步筛选:筛选所有亮度大于100的点
        self.addr_light= numpy.array(numpy.where(self.image > 150))
        ##位置修正
